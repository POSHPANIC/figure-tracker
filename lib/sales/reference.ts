...
export async function buildReference(
  ...
  const sales = await prisma.sale.findMany({
    // No status filter any more: a Sale row only exists because a moderator
    // approved the report that produced it, so every one of them is already
    // the approved kind. The column went when auto-approval did.
    where: { figureId, condition, soldAt: { gte: since } },
