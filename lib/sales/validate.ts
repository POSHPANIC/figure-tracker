 ...
 *
 * The problem this solves: a published market value that anyone can move is
 * worthless. But holding every report for manual review makes reporting
 * pointless — nobody submits data that takes days to appear.
 *
 * So reports are screened against what we already know about the figure. Ones
 * that look ordinary go live immediately; ones that would move the number a lot
 * wait for a human. The cost of a false hold is a small delay. The cost of a
 * false approve is a corrupted price history that people make buying decisions
 * on, so the thresholds lean toward holding.
 *
